...
class Report_m extends Winter_MVC_Model {
    ...
    public function report_sendemail($report_id)
    {
        ...
        if(method_exists($this, "print_".$report_data->format))
        {
            ...
            $subject = __('Report log', 'winter-activity-log').' '.date('Y-m-d H:i:s');
            $body = __('Report log attached in file', 'winter-activity-log').' '.$report_data->format.' '.$report_data->report_name.' '.date('Y-m-d-H-i-s');
            ...
            if($ret === TRUE)
            {
                ...
                return __('Mail sent successfuly to', 'winter-activity-log').' '.$report_data->report_email;
            }
            else
            {
                return __('Mail sending FAILED', 'winter-activity-log');
            }
        }
        else
        {
            return __('Mail sending format Method not exists', 'winter-activity-log');
        }
    }
