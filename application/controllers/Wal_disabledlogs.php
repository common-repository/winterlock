...
class Wal_disabledlogs extends Winter_MVC_Controller {
	...
	{
        // Load view
        $this->load->view('wal_disabledlogs/index', $this->data);
    }
	...
	public function datatable()
	{
        ...
        // configuration
        $columns = array('idcontrol', 'title', 'date', 'description', 'is_skip');
        $controller = 'control';
        ...
        $recordsTotal = $this->{$controller.'_m'}->total_lang(array('is_skip'=>1), NULL);
        
        wal_prepare_search_query_GET($columns, $controller.'_m');
        $recordsFiltered = $this->{$controller.'_m'}->total_lang(array('is_skip'=>1), NULL);

        wal_prepare_search_query_GET($columns, $controller.'_m');
        $data = $this->{$controller.'_m'}->get_pagination_lang($length, $start, array('is_skip'=>1));
