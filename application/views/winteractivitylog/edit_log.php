...
$options = '';
$options.= wmvc_btn_block(admin_url("admin.php?page=wal_controlsecurity&function=control_log&subfunction=block&log_id=".intval($form_data->idlog))).' ';
$options.= wmvc_btn_hide(admin_url("admin.php?page=wal_controlsecurity&function=control_log&subfunction=hide&log_id=".intval($form_data->idlog))).' ';

if($form_data->is_favourite == 1)
{
    $options.= wmvc_btn_save(admin_url("admin.php?page=winteractivitylog&function=save_log_rem&id=".intval($form_data->idlog)), '').' ';
}
else
{
    $options.= wmvc_btn_save(admin_url("admin.php?page=winteractivitylog&function=save_log&id=".intval($form_data->idlog))).' ';
}
$options.= wmvc_btn_delete(admin_url("admin.php?page=winteractivitylog&function=remove&log_id=".intval($form_data->idlog))).' ';
        ...
        <div class="panel-body">
            ...
            <div>  
                ...
                <?php if(strpos($key, 'data') !== FALSE)continue; ?>
                    ...
                    <tr><th scope="row"><?php echo esc_html($key); ?></th><td><?php echo htmlentities($val); ?></td></tr>
...
<div class="winterlock_wrap">
    <div class="panel panel-default panel_log">
        <div class="panel-heading">
            <h3 class="panel-title"><?php echo __('Track user by ID:','winter-activity-log'); ?> <?php echo esc_html(strip_tags($form_data->user_info)); ?>, <?php echo __('Last 100 logs','winter-activity-log'); ?></h3>
        </div>
        <div class="panel-body">
            <?php

                $this->db->limit(100);
                $log_data = $this->log_m->get_by(array('user_id'=>$form_data->user_id));
                ...
                foreach($log_data as $key=>$row): ?>
                    <tr>
                        <td class="event_td <?php echo esc_attr($sw_log_generate_class($row->description));?>">
                            ...
                            <a target="_blank" href="<?php echo admin_url("admin.php?page=winteractivitylog&function=edit_log&id=".$row->idlog); ?>"><?php echo wp_kses_post($row->description); ?></a>
                        ...
                        <th scope="row"><a target="_blank" href="<?php echo admin_url("admin.php?page=winteractivitylog&function=edit_log&id=".$row->idlog); ?>"><?php echo esc_html($row->date); ?></a></th>
                    </tr>
                <?php endforeach; ?>
                </table>    
                <?php else: ?>
...
<div class="winterlock_wrap">
    <div class="panel panel-default panel_log">
        <div class="panel-heading">
            <h3 class="panel-title"><?php echo __('Track user by IP:','winter-activity-log'); ?> <?php echo esc_html(strip_tags($form_data->ip)); ?>, <?php echo __('Last 100 logs','winter-activity-log'); ?></h3>
        </div>
        <div class="panel-body">
            <?php

                $this->db->limit(100);
                $log_data = $this->log_m->get_by(array('ip'=>$form_data->ip));
                $avatar = '<span class="dashicons dashicons-before dashicons-admin-users wal-system-icon"></span>';
